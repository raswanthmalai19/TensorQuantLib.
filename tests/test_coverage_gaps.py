...
from tensorquantlib.__main__ import main, build_parser


class TestCLIPrice:
    def test_bs_price(self, capsys):
        rc = main(["price", "--S", "100", "--K", "100", "--T", "1", "--r", "0.05", "--sigma", "0.2"])
        assert rc == 0
        out = capsys.readouterr().out
        assert "Price" in out

    def test_bs_put(self, capsys):
        rc = main(["price", "--S", "100", "--K", "100", "--T", "1", "--r", "0.05", "--sigma", "0.2", "--type", "put"])
        assert rc == 0
...
class TestCLIAmerican:
    def test_american(self, capsys):
        rc = main(["american", "--S", "100", "--K", "100", "--T", "1",
                    "--r", "0.05", "--sigma", "0.2", "--paths", "5000", "--steps", "50", "--seed", "42"])
        assert rc == 0
        out = capsys.readouterr().out
...
class TestCLIHeston:
    ...
    def test_heston_put(self, capsys):
        rc = main(["heston", "--S", "100", "--K", "100", "--T", "1", "--r", "0.05", "--type", "put"])
        assert rc == 0


class TestCLIAsian:
    def test_asian_arithmetic(self, capsys):
        rc = main(["asian", "--S", "100", "--K", "100", "--T", "1",
                    "--r", "0.05", "--sigma", "0.2", "--paths", "5000", "--steps", "50", "--seed", "42"])
        assert rc == 0

    def test_asian_geometric(self, capsys):
        rc = main(["asian", "--S", "100", "--K", "100", "--T", "1",
                    "--r", "0.05", "--sigma", "0.2", "--avg", "geometric",
                    "--paths", "5000", "--steps", "50", "--seed", "42"])
        assert rc == 0
        out = capsys.readouterr().out
...
class TestCLIBarrier:
    def test_barrier(self, capsys):
        rc = main(["barrier", "--S", "100", "--K", "100", "--T", "1",
                    "--r", "0.05", "--sigma", "0.2", "--barrier", "90",
...
class TestCLICompareVR:
    def test_compare_vr(self, capsys):
        rc = main(["compare-vr", "--S", "100", "--K", "100", "--T", "1",
                    "--r", "0.05", "--sigma", "0.2", "--paths", "5000"])
        assert rc == 0
...
class TestHestonCoverageGaps:
    ...
    def test_heston_greeks_call(self):
        from tensorquantlib.finance.heston import heston_greeks, HestonParams
...
class TestExoticsCoverageGaps:
    ...
    def test_digital_mc_put(self):
        ...
        p = digital_price_mc(100, 100, 1.0, 0.05, 0.2, option_type="put",
    ...
    def test_barrier_mc_down_in(self):
        ...
        p = barrier_price_mc(100, 100, 1.0, 0.05, 0.2, 90, "down-and-in",
                             n_paths=10_000, seed=42)
        assert p >= 0

    def test_barrier_mc_up_in(self):
        from tensorquantlib.finance.exotics import barrier_price_mc
        p = barrier_price_mc(100, 100, 1.0, 0.05, 0.2, 120, "up-and-in",
                             n_paths=10_000, seed=42)
        assert p >= 0

    def test_lookback_floating(self):
        from tensorquantlib.finance.exotics import lookback_price_mc
        p, se = lookback_price_mc(100, None, 1.0, 0.05, 0.2,
                                  strike_type="floating", n_paths=10_000, seed=42)
        assert p > 0

    def test_lookback_put(self):
        from tensorquantlib.finance.exotics import lookback_price_mc
        p, se = lookback_price_mc(100, 100, 1.0, 0.05, 0.2,
                                  option_type="put", n_paths=10_000, seed=42)
        assert p > 0

    def test_cliquet_negative_floor(self):
        from tensorquantlib.finance.exotics import cliquet_price_mc
        result = cliquet_price_mc(100, 1.0, 0.05, 0.2, n_periods=4,
                             floor=-0.05, cap=0.10, n_paths=10_000, seed=42)
    ...
    def test_rainbow_best_of(self):
        ...
        result = rainbow_price_mc(
            spots=[100, 100], K=100, T=1.0, r=0.05,
            sigmas=[0.2, 0.25], corr=[[1, 0.5], [0.5, 1]],
            rainbow_type="best_of", n_paths=10_000, seed=42,
        )
        p = result[0] if isinstance(result, tuple) else result
        assert p > 0
    ...
    def test_rainbow_worst_of(self):
        ...
        result = rainbow_price_mc(
            spots=[100, 100], K=100, T=1.0, r=0.05,
            sigmas=[0.2, 0.25], corr=[[1, 0.5], [0.5, 1]],
            rainbow_type="worst_of", n_paths=10_000, seed=42,
        )
        p = result[0] if isinstance(result, tuple) else result
        assert p > 0
