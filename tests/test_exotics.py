...
class TestAsianOptions:
    ...
    def test_geometric_mc_vs_analytic(self):
        ...
        mc, stderr = asian_price_mc(100, 100, 1.0, 0.05, 0.20, average_type="geometric",
                                     n_paths=200_000, seed=42, return_stderr=True)
...
class TestDigitalOptions:
    ...
    def test_mc_vs_analytic_cash(self):
        ...
        mc, stderr = digital_price_mc(100, 100, 1.0, 0.05, 0.20, n_paths=200_000, seed=0, return_stderr=True)
...
class TestBarrierOptions:
    ...
    def test_mc_down_and_out(self):
        mc, stderr = barrier_price_mc(100, 100, 1.0, 0.05, 0.20, barrier=90,
                                       barrier_type="down-and-out", n_paths=200_000, seed=0,
                                       return_stderr=True)
    ...
    def test_mc_vs_analytic_down_and_out(self):
        ...
        mc, stderr = barrier_price_mc(100, 100, 1.0, 0.05, 0.20, barrier=90,
                                       barrier_type="down-and-out", n_paths=300_000,
                                       n_steps=252, seed=42,
                                       return_stderr=True)
    ...
    def test_mc_put_down_and_in(self):
        mc = float(barrier_price_mc(100, 100, 1.0, 0.05, 0.20, barrier=90,
    ...
    def test_in_out_parity(self):
        ...
        vanilla = float(bs_price_numpy(100, 100, 1.0, 0.05, 0.20))
        din  = barrier_price_mc(100, 100, 1.0, 0.05, 0.20, 90, "down-and-in", n_paths=200_000, seed=7)
        dout = barrier_price_mc(100, 100, 1.0, 0.05, 0.20, 90, "down-and-out", n_paths=200_000, seed=7)
...
class TestLookback:
    ...
    def test_mc_fixed_call(self):
        ...
        price, se = lookback_price_mc(S=100, K=100, T=1.0, r=0.05, sigma=0.2,
                                       strike_type='fixed', seed=42)
        assert price > 0
        assert se < price * 0.1

    def test_mc_floating_call(self):
        from tensorquantlib.finance.exotics import lookback_price_mc
        price, se = lookback_price_mc(S=100, K=None, T=1.0, r=0.05, sigma=0.2,
                                       strike_type='floating', seed=42)
        assert price > 0
...
class TestCliquet:
    ...
    def test_floor_increases_price(self):
        ...
        from tensorquantlib.finance.exotics import cliquet_price_mc
        p_no_floor, _ = cliquet_price_mc(S=100, T=1.0, r=0.05, sigma=0.2,
                                          n_paths=50_000, seed=42)
        p_with_floor, _ = cliquet_price_mc(S=100, T=1.0, r=0.05, sigma=0.2,
                                            floor=0.0, n_paths=50_000, seed=42)
        assert p_with_floor >= p_no_floor - 0.5

    def test_cap_decreases_price(self):
        """Adding a per-period cap should decrease price."""
        from tensorquantlib.finance.exotics import cliquet_price_mc
        p_no_cap, _ = cliquet_price_mc(S=100, T=1.0, r=0.05, sigma=0.2,
                                        n_paths=50_000, seed=42)
        p_with_cap, _ = cliquet_price_mc(S=100, T=1.0, r=0.05, sigma=0.2,
                                          cap=0.02, n_paths=50_000, seed=42)
        assert p_with_cap <= p_no_cap + 0.5

    def test_global_cap(self):
        from tensorquantlib.finance.exotics import cliquet_price_mc
        price, se = cliquet_price_mc(S=100, T=1.0, r=0.05, sigma=0.2,
                                      global_cap=0.1, seed=42)
        assert price >= 0
...
class TestRainbow:
    ...
    def test_best_of_call_positive(self):
        ...
        price, se = rainbow_price_mc(spots, K=100, T=1.0, r=0.05, sigmas=sigmas,
                                      corr=corr, seed=42, n_steps=50)
        assert price > 0
    ...
    def test_best_of_ge_individual(self):
        ...
        sigmas = np.array([0.2, 0.3])
        corr = np.array([[1.0, 0.5], [0.5, 1.0]])
        best_of, _ = rainbow_price_mc(spots, K=100, T=1.0, r=0.05, sigmas=sigmas,
                                       corr=corr, seed=42, n_paths=100_000, n_steps=50)
        # Vanilla calls
    ...
    def test_worst_of_le_individual(self):
        ...
        sigmas = np.array([0.2, 0.3])
        corr = np.array([[1.0, 0.5], [0.5, 1.0]])
        worst_of, _ = rainbow_price_mc(spots, K=100, T=1.0, r=0.05, sigmas=sigmas,
                                        corr=corr, rainbow_type='worst-of', seed=42,
                                        n_paths=100_000, n_steps=50)
        v1 = float(bs_price_numpy(100, 100, 1.0, 0.05, 0.2))
    ...
    def test_put_positive(self):
        ...
        corr = np.array([[1.0, 0.5], [0.5, 1.0]])
        price, se = rainbow_price_mc(spots, K=100, T=1.0, r=0.05, sigmas=sigmas,
                                      corr=corr, option_type='put', seed=42, n_steps=50)
        assert price > 0
